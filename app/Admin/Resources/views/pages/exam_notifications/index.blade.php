...
@section('header')
    <h1 class="page-title">Exam Notifications</h1>
    <div class="page-header-actions">
        @can('create', \App\Models\ExamNotification::class)
        <a class="btn btn-sm btn-primary btn-round" href="{{ route('admin.exam-notifications.create') }}">
    ...
    <div class="card">
        <div class="card-body bg-grey-100">
            <form class="form-inline mb-0" id="form-filter">
                ...
                <div class="form-group">
                    <input class="form-control w-full" id="exam-notification-last-date" type="date" name="last_date" placeholder="Date" autocomplete="off">
                </div>
                <div class="form-group">
        ...
        <div class="card-body">
            {!! $table->table(['id' => 'table-exam-notifications'], true) !!}
        </div>
    </div>
@endsection
...
@push('scripts')
    <script>
        $(function() {
            let $table = $('#table-exam-notifications');
                    ...
                    $.ajax({
                        ...
                        if(data){
                            if(data.is_published==1){
                                toastr.success('Exam Notification published');
                            }
                            else{
                                toastr.success('Exam Notification unpublished');
                            }
            ...
            $table.on('preXhr.dt', function (e, settings, data) {
                data.filter = {
                    search: $('#search').val(),
                    date: $('#exam-notification-last-date').val(),
                };
            });
            ...
            $table.on('click', '.button-delete', function (e) {
                ...
                alertify.confirm("Are you sure?", function () {
                    $.ajax({
                        ...
                        toastr.success('Exam Notification Successfully deleted');
                        $table.DataTable().draw();
                    }).fail(function(jqXHR, textStatus, errorThrown) {
