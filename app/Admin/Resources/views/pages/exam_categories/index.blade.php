...
@section('header')
    <h1 class="page-title">Exam Categories</h1>
    <div class="page-header-actions">
        @can('create', \App\Models\ExamCategory::class)
        <a class="btn btn-sm btn-primary btn-round" href="{{ route('admin.exam-categories.create') }}">
...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            {!! $table->table(['id' => 'table-exam-categories'], true) !!}
        </div>
    </div>
@endsection
...
@push('scripts')
    <script>
        $(function() {
            let $table = $('#table-exam-categories');
            ...
            $table.on('click', '.button-delete', function (e) {
                ...
                alertify.confirm("Are you sure?", function () {
                    $.ajax({
                        ...
                        toastr.success('Exam Category Successfully deleted');
                        $table.DataTable().draw();
            ...
            $(function() {
                $('#table-exam-categories tbody').sortable({
                    update: function() {
                        let exam_category;
                        let exam_categories = [];

                        $(this).find('tr').each(function() {
                            exam_category = $(this).find('.exam_category-id').val();
                            exam_categories.push(exam_category);
                        });

                        $.ajax({
                            url: '{{ url('change-exam_category-order') }}',
                            data: {
                                exam_categories: exam_categories
                            }
                        }).done(function() {
                            $('#table-exam-categories').DataTable().draw();
                        });
                    }
                });
            });
