...
@section('content')
    <div class="panel">
        <form id="form-update" method="POST" action="{{ route('admin.exam-categories.update', $exam_category->id) }}">
            @csrf
            @method('PUT')
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <input class="form-control @error('name') is-invalid @enderror" id="name" name="name"
                                       placeholder="Name" value="{{ old('name', $exam_category->name) }}" autocomplete="off">
                                @error('name')
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-9">
                                <input class="form-control @error('order') is-invalid @enderror" id="order" name="order"
                                       placeholder="Order" value="{{ old('order', $exam_category->order) }}" autocomplete="off">
                                @error('order')
                                <span class="invalid-feedback" role="alert">{{ $message }}</span>
                                @enderror
                            </div>
