        ...
        <div class="col-lg-12">
            <ul class="pagination justify-content-center d-flex pt-5">
                ...
                @else
                    <li class="page-item m-1">
                        <a class="page-link rounded-lg btn-round-md p-0 fw-600 shadow-xss bg-white text-grey-900 border-0"
                           href="{{url('user/tests/'.$test_id.'?page='.($paginator->currentPage()-1).'&package='.$package_id.'&question='.($question-1))}}" tabindex="-1" aria-disabled="true">
                            <i class="ti-angle-left"></i>
                        </a>
                    </li>
                @endif
                            ...
                            @else
                                ...
                                <li class="page-item m-1">
                                    <a class="page-link  bg-white text-grey  rounded-lg btn-round-md p-0 fw-600 font-xssss shadow-xss border-0"
                                       href="{{url('user/tests/'.$test_id.'?page='.$page.'&package='.$package_id.'&question='.($question+1))}}">
                                        {{ $page }}
                                    </a>
                                </li>
                            @endif
                ...
                @if ($paginator->hasMorePages())
                 ...
                 border-0" href="{{url('user/tests/'.$test_id.'?page='.$page.'&package='.$package_id.'&question='.($question+1).'&tab='.$tab)}}">
