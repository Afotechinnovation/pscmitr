                    ...
                    <div class="card d-block w-100 border-0 shadow-xss rounded-lg overflow-hidden p-4">
                        ...
                        <div class="row justify-content-center">
                            <div class="page-title style1 col-xl-6 col-lg-6 col-md-10 text-center mb-5">
                                <h2 class="text-grey-900 fw-700 display1-size display2-md-size pb-3 mb-0 d-block">Live Tests</h2>
                            </div>
                        </div>
                        @if($availableLiveTestCount > 0)
                            @if($liveTests->count())
                                <div class="row card-scroll">
                                     @foreach( $liveTests as $liveTest )
                                        @if($liveTest->is_live_now)
                                            <div class="col-xs-12 col-sm-12 col-md-4 mb-3">
                                                ...
                                                <div class="card-image w-100 ">
                                                    @if(\Illuminate\Support\Facades\Auth::user())
                                                        <a href="#" data-package-test-id="{{ $liveTest->test_id }}" data-package-id ="{{ $liveTest->package_id }}" class="purchase-alert position-relative d-block">
                                                            <img src="{{ $liveTest->image}}" alt="image" class="w-100">
                                                        </a>
                                                    @else
                                                        <a data-toggle="modal" data-target="#Modallogin" data-redirect-page="liveTest" class="login-section position-relative d-block">
                                                            <img src="{{  $liveTest->image }}" alt="image" class="w-100">
                                                        </a>
                                                    @endif
                                                </div>
                                                <div class="card-body p-2">
                                                    <div class="row p-2">
                                                        <div class="col-4">
                                                            <span class="font-xssss text-grey-500 fw-600"><i class="ti-time mr-2"></i>{{ $liveTest->total_time }} </span>
                                                        </div>
                                                        <div class="col-5 p-2">
                                                        ...
                                                        <div class="col-3 p-1">
                                                            <span class="font-xssss text-grey-500 float-left fw-600">
                                                                {{date('h:i A', strtotime(Carbon\Carbon::parse($liveTest->live_test_date_time )->toTimeString())) }}
                                                            </span>
                                                        </div>
                                                    </div>
                                                    <div class="p-2 test-fixed-height" >
                                                        <h6 class="author-name font-xsss fw-600 mb-0 text-grey-800">
                                                           <span class=" font-xssss text-black-50">{{ \Illuminate\Support\Str::limit($liveTest->display_name, 25, $end=' ...') }}</span>
                                                        </h6>
                                                    </div>
                                                    <div class="row pl-2">
                                                        <div class="col-6">
                                                         <span class="font-xssss text-grey-700 float-left fw-600 d-inline-block ">
                                                           {{ $liveTest->total_questions }}  Questions
                                                        </span>
                                                        </div>
                                                        <div class="col-6">
                                                             <span class="font-xssss float-right  fw-600 d-inline-block ">
                                                               {{ $liveTest->total_marks }}  Marks
                                                            </span>
                                                        </div>
                                                    </div>
                                                    ...
                                                    <div class="p-2 text-center">
                                                        @if( \Illuminate\Support\Facades\Auth::user() )
                                                            <a href="#"   data-test-id ="{{ $liveTest->id  }}"  data-package-id ="{{ $liveTest->package_id }}"
                                                               class="test-detail-modal border-0 p-2 d-inline-block text-white fw-700 lh-27 rounded-lg w125 font-xsssss ls-3 bg-current">
                                                               START
                                                            </a>
                                                        @else
                                                            <button  data-toggle="modal" data-target="#Modallogin" data-redirect-page="liveTest"
                                        ...
                                        @else
                                           <div class="text-center"> You have Already Attended all live tests.</div>
                                        @endif
                                    @endforeach
                                </div>
                            @else
                            ...
                            @endif
                        @else
                            <center> No Live Tests available</center>
                        @endif
                    </div>
                </div>
            </div>
