...
var _imatheq_save_eq_callback = null;
 ...
 */
function imatheq_openEditor(hostEditor, lang, isIframe, newFormula, save_callback) {

	if(lang === null || lang === undefined) {
		lang = "en";
	}
	
	if(save_callback) _imatheq_save_eq_callback = save_callback;
	...
	var editor_name = location.protocol == 'https:' ? "editor_https.html" : "editor.html";
	...
	var mml = newFormula ? newFormula : "";
	var iMathEQ_qid = "";

    var title = 'iMathEQ Math Equation Editor';
	if(_iamtheq_popup_atttributes === undefined || _iamtheq_popup_atttributes === null) {
		_iamtheq_popup_atttributes = 'menubar=1,resizable=1,width=800,height=600';
	}
    if (typeof _imatheq_conf_modal_window != 'undefined' && _imatheq_conf_modal_window === false) {
        //_imatheq_popupWindow = window.open(path, title, _iamtheq_popup_atttributes);
...
ModalWindow.prototype.create = function(lang) {
	...
	oRestore.title = _imatheq_strings["maximize"];
	...
	oiframe.src = imatheq_getCommonPath() + "/common/editor.html?lang=" + lang + "&amp;dir=ltr"
...
_imatheq_plugin_loaded = function() {
	return (_imatheq_css_loaded &&
			_imatheq_conf_loaded &&
			_imatheq_langstr_loaded
			);
}